...
public class AIPathModified : AIBaseModified {
	...
	public virtual void SearchPath () {
		if (this.targetPosition == null) throw new System.InvalidOperationException("Target is null");

		lastRepath = Time.time;
		// This is where we should search to
		Vector3 targetPositionLocal = (Vector3)this.targetPosition;
		...
		seeker.StartPath(GetFeetPosition(), targetPositionLocal);
	}
	...
	protected override void MovementUpdate (float deltaTime) {
		...
		if (!interpolator.valid) {
			...
			rotationAfterMove = RotateTowards(velocity2D, currentRotationSpeed * deltaTime);

			var delta2D = CalculateDeltaToMoveThisFrame(movementPlane.ToPlane(currentPosition), distanceToEnd, deltaTime);
			positionAfterMove = Move(currentPosition, movementPlane.ToWorld(delta2D, verticalVelocity * deltaTime));
